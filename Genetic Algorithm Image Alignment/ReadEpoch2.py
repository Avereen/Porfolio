...
#     dim = sum([dims[0]**.5,dims[1]**.5])/2
#     designs = population.generateDesign(popOrder[0])
#     divisors=lambda x : [(i+2,x/(i+2)) for i in range(int(x/2)) if x%(i+2)==0]
#     sa1 = im.surfaceIntegrationPlane(designs[0])
#     sa2 = im.surfaceIntegrationPlane(designs[1])
#     return sa1, sa2
...
if __name__ == "__main__": 
    ...
    fig.savefig('prof_combine'+'.png',dpi=300)
